...
$excelFilePath1 = 'excel1.xls'; // (EXAMPLE)
$excelFilePath2 = 'excel2.xls'; // (EXAMPLE)

// Tentukan path untuk menyimpan file CSV
$csvFilePath = 'excel_merge.xls';

if(empty($_FILES['fileUpload']['name'])){
    echo '<script>';
    echo 'alert("File excel utama tidak ada");';
...
if(empty($_FILES['fileUpload2']['name'])){
    echo '<script>';
    echo 'alert("File excel yang akan digabung tidak ada");';
    echo 'window.history.back();'; // Mengarahkan ke halaman sebelumnya
    echo '</script>';
    return false;
}
...
if(isset($_FILES['fileUpload'])) {
  ...
  $newFileName = $excelFilePath1; //uniqid('file_') . '.' . $extension;
  ...
  if(in_array($extension, $allowedExtensions)) {
    if (file_exists($excelFilePath1)) {
      unlink($excelFilePath1);
    }
...
if(isset($_FILES['fileUpload2'])) {
  ...
  // Tentukan nama file baru
  $extension = pathinfo($file['name'], PATHINFO_EXTENSION);
  $newFileName = $excelFilePath2; //uniqid('file_') . '.' . $extension;
  ...
  // Cek apakah ekstensi file diizinkan
  if(in_array($extension, $allowedExtensions)) {
    if (file_exists($excelFilePath2)) {
      unlink($excelFilePath2);
    }
    // if (file_exists($csvFilePath)) {
    //   unlink($csvFilePath);
    // }
...
$data22 = $sheet2->toArray();
// OLAH DATA

$dataItem2 = array();
$result22 = array_map(function($array22) {
    return array($array22[1], $array22[3]);
}, $data22);
$masterCOA = array_slice($result22, 9);
...
// Load file Excel 1
$spreadsheet = IOFactory::load($excelFilePath1);
// Ambil data dari sheet pertama (indeks 0)
...
foreach($modifiedArray as $subarray){
  ...
  if(count($filteredArray) > 5){
    ...
    foreach ($subarray as $keyItem => $item) {
      ...
      // GET KETERANGAN 2
      if($keyItem == $indexCol4){ // GET NO BUKTI
        $resultCOA = "";
        foreach ($masterCOA as $itemCOA) {
            if ($itemCOA[0] == $item) {
                $resultCOA = $itemCOA[1];
                break;
            }
        }
        $htmlTable .= '<td '.$colNoBukti.'>'.$resultCOA.'</td>';
      }
